...
class Form extends Controller{
    ...
    constructor(model, configs={}){
        ...
        this.deletes = configs.hasOwnProperty('deletes') ? configs.deletes : true;
        ...
        this.action = "";
        this.csrfToken = null;
        this.actionbars = configs.actionbars && configs.actionbars.length ? configs.actionbars : [];
    ...
    normalizeItems(items){
        ...
        return Object.keys(items).map(el => {
            ...
            if(item.items){
                let editMode = item.hasOwnProperty('disabled') ? (item.disabled ? false : true) : item.editMode;
                item.items = this.normalizeItems(item.items);
                item.editMode = editMode;
                
                if(!item.editMode){
                    let mapx = item.items.map(xt => ({key: xt.key, type: xt.type}));
                    if(item.value && Array.isArray(item.value)){
                        item.value = item.value.map(elm => {
                            let prop = Object.keys(elm).forEach(v => {
                                let itm = mapx.find(x => x.key === v);
                                let type = itm ? itm.type : null;
                                switch(type){
                                    case 'date':
                                        elm[v] = new Date(elm[v]).toLocaleString(undefined, {year: 'numeric', month: 'long', day: 'numeric'});
                                        break;
                                    case 'datetime':
                                        elm[v] = new Date(elm[v]).toLocaleString(undefined, {year: 'numeric', month: 'long', day: 'numeric', hour: 'numeric', minute: 'numeric'});
                                        break;
                                }
                            });
    ...
    getActionbars(){
        ...
        if(this.actionbars.length){
            const args = {
                doc: this.data, 
                userAccess: this.userAccess, 
                hasRoles: this.haveRoles, 
                editMode: this.editMode,
                vars: this.vars
            };
            
            this.actionbars.forEach(action => {
                if(action.type === 'edit'){
                    action = {
                        type: 'linkButton', 
                        text: action.hasOwnProperty('text') ? action.text : 'Edit', 
                        icon: action.hasOwnProperty('icon') ? action.icon : 'edit', 
                        location: this.path + '?OpenForm&id=' + this.data._id, 
                        target: '_self',
                        hide: action.hide
                    };
                }

                action.location = action.hasOwnProperty('location') ? (typeof action.location === 'function' ? action.location(args) : action.location) : null;
                
                let hide = action.hasOwnProperty('hide') ? (typeof action.hide === 'function' ? action.hide(args) : action.hide) : false;
                if(!hide) actions.push(action);
            });
        }else{
            const authorizedAuthors = typeof this.authors === 'function' ? (this.data._id ? this.authors({doc: this.data, userAccess: this.userAccess, hasRoles: this.haveRoles}) : true) : this.authors;
            if(this.editMode){
                ...
                if(authorizedAuthors){
                    ...
                    const authorizedDeletes = typeof this.deletes === 'function' ? (this.data._id ? this.deletes({doc: this.data, userAccess: this.userAccess, hasRoles: this.haveRoles}) : true) : this.deletes;
                    if(authorizedDeletes){
        ...
        for(let item in this.items){
            ...
            switch(attribute.type){
                ...
                case 'model':
                    ...
                    break;
                case 'items':
                    serialized[item] = body[item] ? JSON.parse(body[item]) : [];
                    break;
                default:
    ...
    async fn(req, res, next){
        ...
        if(req.method === "GET" || req.method === "POST"){
            ...
            if(req.method === "GET"){
                ...
                if(doc){
                    ...
                    return this.callback(req, res, next, this.getFormConfig());
                }
            }
        }
        ...
        if(req.method === "DELETE"){
            ...
            for(let id of ids){
                ...
                const authorizedAuthors = typeof this.authors === 'function' ? (doc._id ? this.authors({doc: doc, userAccess: this.userAccess, hasRoles: this.haveRoles}) : true) : this.authors;
                const authorizedDeletes = typeof this.deletes === 'function' ? (doc._id ? this.deletes({doc: doc, userAccess: this.userAccess, hasRoles: this.haveRoles}) : true) : this.deletes;
                if(authorizedAuthors && authorizedDeletes){
