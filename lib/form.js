...
class Form extends Controller{
        ...
        if(vars){
            ...
            for(let v in vars){
                ...
                if(typeof item === "function"){
                    if(item[Symbol.toStringTag] === 'AsyncFunction'){
                        this.variables[v] = await item();
                    }else{
                        this.variables[v] = item();
                    }
                }else{
                    this.variables[v] = item;
                }
    ...
    normalizeItems(items){
        return Object.keys(items).map(el => {
            ...
            item.required = item.hasOwnProperty('required') ? (typeof item.required === 'function' ? item.required(this.data, this.variables) : item.required) : false;
            item.disabled = item.hasOwnProperty('disabled') ? (typeof item.disabled === 'function' ? item.disabled(this.data, this.variables) : item.disabled) : false;
            item.hide = item.hasOwnProperty('hide') ? (typeof item.hide === 'function' ? item.hide(this.data, this.variables) : item.hide) : false;
            if(item.options && typeof item.options === 'function') item.options = item.options(this.data, this.variables);
