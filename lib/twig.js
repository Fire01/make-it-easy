...
const Twig = {
    ...
    form_row(item, editMode = true) {
        if (item.hasOwnProperty('hide') && item.hide) return "";
    ...
    widget(config) {
        config.value = config.hasOwnProperty('value') && !(config.value === null || typeof config.value === "undefined") ? config.value : '';
    ...
    items(config) {        
        ...
        config.editMode = config.hasOwnProperty('disabled') ? (config.disabled ? false : true) : config.editMode;
