...
class View extends Controller{
    ...
    fn(req, res, next){       
        if(!req.query.hasOwnProperty('json')){
            const canDelete = typeof this.deletes === 'function' ? this.deletes({userAccess: this.userAccess, hasRoles: this.haveRoles}) : this.deletes;
            return res.render('_base/view', {title: this.title, configs : this.getDataTablesConfig(), form_path: this.form ? this.form.path : null, canDelete: canDelete});
        }else{
            if(!this.form) this.links = [];
            let map = this.column.map(el => el.data).concat(['_id']);
            let viewConditions = typeof this.conditions === 'function' ? this.conditions({userAccess: this.userAccess, hasRoles: this.haveRoles}) : this.conditions;
            
            this.model.find(viewConditions)
            .then((result) => {
                return res.json({
